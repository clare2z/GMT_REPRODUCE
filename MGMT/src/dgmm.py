...
class DGMMFramework:
    def __init__(self, encoder_hidden_dim=128, encoder_output_dim=64,
        ...
        # MGMT interface compatibility (do not affect DGMM core logic)
    ...
    def _group_layers(self, grads: Dict[str, torch.Tensor], skip_names: Set[str]) -> Dict[str, torch.Tensor]:
        ...
        for name, grad in grads.items():
            if name in skip_names:
                continue
            # DGMM statistics only: offload to CPU to avoid GPU memory spike
            g = grad.detach().float().cpu().flatten()
